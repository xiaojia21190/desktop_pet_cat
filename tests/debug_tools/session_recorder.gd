...
var _session  # FocusSessionMode 引用
...
func _ready() -> void:
	layer = 100
	_trash_purge_old_days = maxi(_session.trash_purge_old_default_days, 1)
...
func start_recording_script(duration_seconds: int = -1, force_demo_mode: bool = true) -> void:
	...
	if duration <= 0:
		duration = _session.recording_default_duration_seconds
	_session._recording_mode = true
	_session._recording_target_seconds = max(duration, 10)
	_session._recording_events.clear()
	_session._recording_output_path = ""
	_session._replay_mode = false
	_session._replay_source_path = ""

	if force_demo_mode and not _session.demo_script_mode:
		_session.set_demo_script_enabled(true, false)

	_session.start_session()
	_session._record_event("recording_start", {
		"target_seconds": _session._recording_target_seconds,
		"demo_mode": _session.demo_script_mode
	})
	_session._hud.set_hint("Recording script started.")
	_refresh_demo_control_ui()

func stop_recording_script(reason: String = "manual_stop") -> void:
	if not _session._recording_mode:
		return
...
func export_recording_log(custom_path: String = "") -> String:
	...
	var payload := {
		...
		"target_seconds": _session._recording_target_seconds,
		"demo_mode": _session.demo_script_mode,
		"events": _session._recording_events
	}
...
func _replay_selected_or_latest() -> bool:
	...
	if replay_recording_log(latest_path):
		_session._hud.set_hint("Selected replay failed, fallback to latest: " + _short_recording_path(latest_path))
		return true
	return false
...
func replay_recording_payload(payload: Dictionary, source_tag: String = "") -> bool:
	...
	_session.set_demo_events(replay_events)
	_session.set_demo_script_enabled(true, false)
	_session._replay_mode = true
	_session._replay_source_path = source_tag
	_session.start_session()
	_session._hud.set_hint("Replay loaded: " + _short_recording_path(_session._replay_source_path))
	_refresh_demo_control_ui()
	return true
...
func _cycle_demo_speed() -> void:
	...
	_session._hud.set_hint("Replay speed: " + str(_demo_playback_speed) + "x")
	_refresh_demo_control_ui()
...
func _refresh_ops_panel() -> void:
	...
	if not _ops_panel_visible:
		return
	var max_lines := maxi(_session.recording_ops_panel_preview_size, 1)
...
func _finalize_recording(reason: String) -> void:
	...
	_session._record_event("recording_finalize", {"reason": reason})

	if _session.recording_auto_export:
		_session._recording_output_path = export_recording_log()
		if not _session._recording_output_path.is_empty():
			_refresh_recording_list(true)
			_select_recording_path(_session._recording_output_path)
	else:
		_session._recording_output_path = ""

	var summary := _build_recording_summary(reason)
	recording_script_finished.emit(_session._recording_output_path, summary)

	if _session._recording_output_path.is_empty():
		_session._hud.set_hint("Recording finished: " + reason)
	else:
		_session._hud.set_hint("Recording exported: " + _session._recording_output_path)

	_session._recording_mode = false
	_refresh_demo_control_ui()

func _build_recording_summary(reason: String) -> Dictionary:
	return {
		"reason": reason,
		"duration_seconds": _session._elapsed_seconds,
		"target_seconds": _session._recording_target_seconds,
		"events_count": _session._recording_events.size(),
		"output_path": _session._recording_output_path
	}

func _refresh_demo_control_ui() -> void:
	if not _demo_status_label:
		return
	var mode_text := "ON" if _session.demo_script_mode else "OFF"
	var pause_text := "PAUSED" if _session._demo_paused else "RUN"
	var recording_text := "REC OFF"
	if _session._recording_mode:
		recording_text = "REC " + str(_session._elapsed_seconds) + "/" + str(_session._recording_target_seconds)
	var replay_text := "REPLAY OFF"
	if _session._replay_mode:
		replay_text = "REPLAY " + _short_recording_path(_session._replay_source_path)
	...
	if _ops_panel_visible:
		ops_text = "OPS ON"
	_demo_status_label.text = "Mode " + mode_text + " | " + pause_text + " | " + recording_text + " | " + replay_text + " | " + speed_text + " | " + selected_text + " | " + trash_text + " | " + delete_text + " | " + purge_text + " | " + purge_old_text + " | " + restore_text + " | " + ops_text + " | event " + str(_session._demo_cursor) + "/" + str(_session._demo_events.size())

func _on_demo_start_pressed() -> void:
	_session.start_demo()

func _on_demo_pause_pressed() -> void:
	_session.pause_demo(not _session._demo_paused)

func _on_demo_next_pressed() -> void:
	if not _session.trigger_next_demo_event():
		_session._hud.set_hint("No pending demo event.")

func _on_demo_record_pressed() -> void:
	if _session._recording_mode:
		stop_recording_script("manual_stop")
		return
	start_recording_script(_session.recording_default_duration_seconds, true)

func _on_demo_replay_pressed() -> void:
	if not _replay_selected_or_latest():
		_session._hud.set_hint("Replay failed: no valid recording log found.")

func _on_demo_export_pressed() -> void:
	if _session._recording_events.is_empty():
		_session._hud.set_hint("Export skipped: no recording events.")
		return
	var path := export_recording_log()
	if path.is_empty():
		_session._hud.set_hint("Export failed.")
		return
	_session._recording_output_path = path
	_refresh_recording_list(true)
	_select_recording_path(path)
	_session._hud.set_hint("Exported: " + _short_recording_path(path))
	_refresh_demo_control_ui()
...
func _on_demo_refresh_pressed() -> void:
	...
	_refresh_trash_list(false)
	if _recording_files.is_empty():
		_session._hud.set_hint("No recording files found.")
	else:
		_session._hud.set_hint("Recording list refreshed: " + str(_recording_files.size()) + " files.")
...
func _on_recording_selected(index: int) -> void:
	...
	_session._hud.set_hint("Selected: " + _short_recording_path(_selected_recording_path))
	_refresh_demo_control_ui()
...
func _on_demo_delete_pressed() -> void:
	if _selected_recording_path.is_empty():
		_session._hud.set_hint("Delete skipped: no selected recording.")
		return
	var now := int(Time.get_unix_time_from_system())
	if _delete_confirm_path != _selected_recording_path or now > _delete_confirm_until:
		...
		_session._hud.set_hint("Press Delete again in 4s: " + _short_recording_path(_selected_recording_path))
		_refresh_demo_control_ui()
		return
	var deleting_path := _selected_recording_path
	if not _delete_recording_path(deleting_path):
		_session._hud.set_hint("Delete failed: " + _short_recording_path(deleting_path))
		_clear_delete_confirmation()
		_refresh_demo_control_ui()
		return
	if _session._replay_source_path == deleting_path:
		_session._replay_source_path = ""
	_clear_delete_confirmation()
	_clear_purge_confirmation()
	_clear_purge_old_confirmation()
	...
	_select_trash_path(_last_deleted_trash_path)
	_session._hud.set_hint("Moved to trash: " + _short_recording_path(deleting_path))
	_refresh_demo_control_ui()

func _on_demo_restore_pressed() -> void:
	var restored_path := _restore_last_deleted_recording()
	if restored_path.is_empty():
		_session._hud.set_hint("Restore failed: no available trashed recording.")
		_refresh_demo_control_ui()
		return
	_clear_delete_confirmation()
	_clear_purge_confirmation()
	...
	_session._hud.set_hint("Restored: " + _short_recording_path(restored_path))
	_refresh_demo_control_ui()
...
func _on_trash_refresh_pressed() -> void:
	...
	_refresh_trash_list(false)
	if _trash_files.is_empty():
		_session._hud.set_hint("Trash is empty.")
	else:
		_session._hud.set_hint("Trash refreshed: " + str(_trash_files.size()) + " files.")
...
func _on_trash_selected(index: int) -> void:
	...
	_session._hud.set_hint("Trash selected: " + _short_recording_path(_selected_trash_path))
	_refresh_demo_control_ui()

func _on_trash_restore_pressed() -> void:
	if _selected_trash_path.is_empty():
		_session._hud.set_hint("Restore skipped: no selected trash file.")
		return
	var restored_path := _restore_trash_path(_selected_trash_path)
	if restored_path.is_empty():
		_session._hud.set_hint("Restore failed: " + _short_recording_path(_selected_trash_path))
		_refresh_demo_control_ui()
		return
	_clear_purge_confirmation()
	_clear_purge_old_confirmation()
	...
	_refresh_trash_list(false)
	_select_recording_path(restored_path)
	_session._hud.set_hint("Restored from trash: " + _short_recording_path(restored_path))
	_refresh_demo_control_ui()

func _on_trash_purge_pressed() -> void:
	if _selected_trash_path.is_empty():
		_session._hud.set_hint("Purge skipped: no selected trash file.")
		return
	var now := int(Time.get_unix_time_from_system())
	if _purge_confirm_path != _selected_trash_path or now > _purge_confirm_until:
		...
		_session._hud.set_hint("Press Purge again in 4s: " + _short_recording_path(_selected_trash_path))
		_refresh_demo_control_ui()
		return
	var purging_path := _selected_trash_path
	if not _purge_trash_path(purging_path):
		_session._hud.set_hint("Purge failed: " + _short_recording_path(purging_path))
		_clear_purge_confirmation()
		_refresh_demo_control_ui()
		return
	_clear_purge_confirmation()
	_clear_purge_old_confirmation()
	_refresh_trash_list(false)
	_session._hud.set_hint("Purged: " + _short_recording_path(purging_path))
	_refresh_demo_control_ui()
...
func _on_trash_purge_old_pressed() -> void:
	...
	if _purge_old_confirm_days != target_days or now > _purge_old_confirm_until:
		...
		_session._hud.set_hint("Press PurgeOld again in 4s: older than %dd" % target_days)
		_refresh_demo_control_ui()
		return
	_clear_purge_old_confirmation()
	_clear_purge_confirmation()
	...
	_refresh_trash_list(false)
	if purged_count <= 0:
		_session._hud.set_hint("PurgeOld finished: no files older than %dd." % target_days)
	else:
		_session._hud.set_hint("PurgeOld removed %d files older than %dd." % [purged_count, target_days])
	_refresh_demo_control_ui()

func _on_ops_tail_pressed() -> void:
	var tail_size := maxi(_session.recording_ops_tail_preview_size, 1)
	var ops_tail := _tail_recording_ops(tail_size)
	if ops_tail.is_empty():
		_session._hud.set_hint("Ops tail empty.")
		return
	...
	_session._hud.set_hint("Ops tail " + str(ops_tail.size()) + ": " + action + " " + result + " " + short_path)
	_refresh_ops_panel()

func _on_ops_panel_pressed() -> void:
	_ops_panel_visible = not _ops_panel_visible
	_refresh_ops_panel()
	_session._hud.set_hint("Ops panel: " + ("ON" if _ops_panel_visible else "OFF"))
	_refresh_demo_control_ui()

func _on_ops_export_pressed() -> void:
	var path := export_recording_ops_log()
	if path.is_empty():
		_session._hud.set_hint("Ops export failed: no ops log.")
		return
	_session._hud.set_hint("Ops exported: " + _short_recording_path(path))
	_refresh_ops_panel()
...
func _on_demo_open_folder_pressed() -> void:
	...
	_session._hud.set_hint("Opened recordings folder.")
